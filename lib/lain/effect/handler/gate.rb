...
module Lain
  module Effect
    class Handler
      ...
      # uses: Gate intercepts what it cares about and lets everything else fall
      # through to `inner` untouched (see {Handler#call}). It IS a gate -- the name
      # says what it does, where the old "Approving" only named the verb.
      #
      ...
      # Gate needing to know why.
      #
      # Gate holds NO Toolset of its own. It asks its `inner` handler what a name
      # resolves to ({Handler#tool_named}), so the tier it gates on is read from
      # the exact tool the executor will dispatch. A second Toolset reference here
      # could diverge from the executor's, and then the gate would consult one map
      # while dispatch used another -- authorization decided against a different
      # set than possession, the very failure "tools are capabilities, not
      # permissions" exists to prevent. One map, by construction.
      #
