...
module Lain
  class Role
    ...
    module Catalog
      ...
      BUILT_INS = [
        Role.new(name: :dev, only: %i[read_file list_files edit_file todo_write bash]),
        Role.new(name: :test_engineer, only: %i[read_file list_files edit_file todo_write bash]),
        ...
        Role.new(name: :researcher, only: %i[read_file list_files]),
