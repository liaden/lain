...
# (`Chat::MessageFormatting`), not a reopening of `Chat`: turning one
# {Message} into the Hash Anthropic's `messages` array expects, including the
# extended-thinking block a `tool_use` or plain-text message must carry first
# when thinking is enabled. `Chat` delegates to it (`MessageFormatting.
# format_message(...)`); nothing here lands on `Anthropic` as a mixed-in
# method. All methods are stateless transformations, so `module_function`
# module methods are the whole surface.
  ...
  class Provider
    module HTTP
      module Providers
        class Anthropic
          module Chat
            # Message-to-wire-block formatting for the Anthropic messages array.
