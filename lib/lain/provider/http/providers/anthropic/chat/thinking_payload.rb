...
# (`Chat::ThinkingPayload`), not a reopening of `Chat`: extended-thinking
# payload construction. Anthropic wants either a token `budget` (`thinking:
# {type: "enabled", budget_tokens:}`) or an `effort` level (`thinking: {type:
# "adaptive"}, output_config: {effort:}`), and which one a model accepts is
# itself a model capability this slice has no registry for, so it asks the
# duck-typed `model` directly via `#reasoning_option`. `Chat` delegates to it
# (`ThinkingPayload.add_thinking_fields(...)`).
  ...
  class Provider
    module HTTP
      module Providers
        class Anthropic
          module Chat
            # Extended-thinking request-payload construction.
            module ThinkingPayload
              module_function
