...
module Lain
  ...
  # {Lain::Effect::Handler}; the handler is the only thing that touches the world.
  # That split is what makes deterministic replay a *recorded handler* rather than
  # a second code path, and what lets an approval or timeout wrap an intention
  ...
  module Effect
    # Kind predicates, defaulting to false. A reader asks an effect *what it is*
    # (`effect.tool_call?`) rather than matching its class -- Null-Object posture:
    # the questions are total over the vocabulary, so no reading site needs a
    # `respond_to?` guard or a `rescue`. Each concrete effect overrides only its
    # own predicate; every other kind inherits the honest `false`.
    module Kind
      def tool_call? = false
      def approval? = false
    end
    ...
    #
    # There are two routes into {Lain::Effect::Handler::Gate}, and this wrapper is
    ...
    # gated; a structured `read_file` is not), and Gate reads that answer off the
    # very tool it will dispatch. This wrapper is the second route -- it marks ONE
    # specific call for approval regardless of the tool's own tier, for when
    # something upstream decides a particular invocation needs a human even though
    # the tool would normally run unattended. Wrapping keeps that per-call decision
