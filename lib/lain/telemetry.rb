...
module Lain
  ...
  # The two live-switch flips (/yolo's policy, /model's model), reopening
  # Telemetry once more for the same missing-seam reason every block above does
  # (CLAUDE.md: a tripped Metrics/ModuleLength names a missing seam; here the
  # seam is "the operator-flip evidence stream, not the per-turn/per-request
  # telemetry stream"). Each is a DUMB CARRIER: the switch that emits it
  # ({Approval::PolicySwitch}/{Context::ModelSwitch}) owns the from/to naming
  # and keeps its own live `@current`; the record only serializes the flip. The
  # discriminator strings "policy_switch"/"model_switch" derive from the class
  # basename ({Journalable#journal_type}) and are LOAD-BEARING -- journal
  # readers and replay match on them, so the class names must not drift.
  module Telemetry
  ...
  # D4's one record, reopening Telemetry once more for the same missing-seam
  # reason every block above does (CLAUDE.md: a tripped Metrics/ModuleLength
  # names a missing seam; here the seam is "what became of a worker's committed
  # work, not the lease lifecycle {IsolationLease} already records"). Emitted by
  # {Isolation::Worktree::Handback} itself rather than by a decorator: handback
  # is a one-shot operation whose whole product IS the outcome, so there is no
  # forwarding duck to wrap the way {Isolation::Journal} wraps a backend.
  module Telemetry
    module Guards
      ...
      # telemetry.rb load time, before the isolation/ unit loads (the same
  ...
  # T5's one record, reopening Telemetry once more for the same missing-seam
  # reason every block above does (CLAUDE.md: a tripped Metrics/ModuleLength
  # names a missing seam; here the seam is "the derived context lineage, not the
  # per-turn/per-request telemetry stream"). Emitted by
  # {Compaction::Derivation}, and READ BACK by {Compaction::DerivationAudit} --
  # which matters, because nothing reads a `compaction` record back today and a
  # write-only trace is this subsystem's default failure mode.
  module Telemetry
    module Guards
  ...
  # T4's one record, reopening Telemetry once more for the same missing-seam
  # reason every block above does (CLAUDE.md: a tripped Metrics/ModuleLength
  # names a missing seam; here the seam is "the approval lifecycle's ASKED
  # moment, not the per-turn/per-request telemetry stream"). Emitted by
  # {Approval::Queue#admit}, which until now emitted nothing at all -- the whole
  # lifecycle's only observable was the post-hoc `approval_decision` written in
  # `#settle`'s ensure.
  module Telemetry
    module Guards
