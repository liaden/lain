...
# spec/support loads unconditionally, so a file-scope require would put the
# 33.6MB SDK back into all 8 `rake pspec` workers, which is the cost this whole
...
module Lain
  class Provider
    ...
    # "a Lain::Provider" group -- an oracle that does not itself satisfy the
    # contract could not measure anything.
    #
    ...
    class AnthropicReference < Provider
      ...
      def initialize(client: nil, channel: Channel::Null.instance, **client_options)
        ...
        # is the correctness ORACLE, not a path any run takes: `--provider
        # anthropic` wires {Anthropic} (see CLI::Backend), and nothing in
        # lib/ or exe/ constructs this class at all. At file scope the gem cost
        # every process 23.2MB on top of the Faraday stack the real path
        # already pays for -- 186MB across an 8-worker `rake pspec` -- for a
        # dependency only the parity specs ever construct.
        #
        # Unconditional, not folded into the `client ||` branch below: a spec
        # injecting a double would otherwise leave `::Anthropic::Errors::*`
        # unresolvable in this class's own rescue clauses.
        require "anthropic"
      ...
      # {Anthropic} so the two backends cannot drift apart on the wire.
