...
require "json"
# NOTE: `require "anthropic"` is deliberately NOT here -- see #initialize.
...
module Lain
  class Provider
    ...
    # {Provider::AnthropicReference} is for the direct one. Mantle speaks the plain
    ...
    # auth mode -- which was the sole reason that gem was a runtime dependency.
    # Now that {Bedrock} carries the `--provider bedrock` path (it reads the
    # same two env vars over the vendored Faraday transport) and this class is
    # oracle-only, both `anthropic` and `aws-sdk-core` are test dependencies.
    # See spec/support/provider_oracles/anthropic.rb for the full rationale.
    class BedrockReference < Provider
      ...
      # Provider::AnthropicReference's: Mantle's request validator rejects the tools'
      ...
      def initialize(client: nil, **client_options)
        ...
        # `anthropic` gem is 23.2MB on top of the Faraday stack, and the DEFAULT
        # path never touches it. Unlike the oracle, this class IS reachable
        # (`--provider bedrock`, CLI::Backend) -- so the load moves from every
        # boot to the one run that selected Bedrock, which needs it anyway.
        # {DEFAULT_MODEL} stays a plain constant, so Backend can read it to
        # resolve a model without paying for the SDK.
        require "anthropic"
