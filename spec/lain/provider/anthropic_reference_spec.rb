...
RSpec.describe Lain::Provider::AnthropicReference do
  ...
  describe "#complete stream_started" do
    ...
    # response. See the identical Anthropic spec for the fuller comment.
  ...
  describe "#complete error wrapping" do
    ...
    it "wraps a non-status SDK error as APIError, still under Lain::Error" do
      ...
      expect { provider.complete(request) }.to raise_error(Lain::Provider::AnthropicReference::APIError) do |wrapped|
        expect(wrapped).not_to be_a(Lain::Provider::AnthropicReference::APIStatusError)
        expect(wrapped.cause).to be(sdk_error)
      end
    end
  end
