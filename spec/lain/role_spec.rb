...
RSpec.describe Lain::Role do
  ...
  let(:union) do
    Lain::Toolset.new(
      %i[read_file list_files glob grep edit_file write_file todo_write bash memory_write memory_read
         web_fetch web_search].map { |n| tool(n) }
    )
  end
  ...
  describe "the catalog ships the OM-5 built-ins" do
    it "names dev, test_engineer, the three reviewers, researcher, court_clerk, auto_approver, harness_improver" do
      expect(Lain::Role::Catalog.names).to contain_exactly(
        ...
        :auto_approver, :harness_improver, :meta_harness, :meta_summarizer, :merge_resolver
      )
    end
